---
title: "Ancient Greece"
year: "2024"
client: "Independent"
discipline: ["Photographer"]
summary: "A quiet index for a design studio, built around severe type, modular imagery, and an archive-first reading rhythm."
order: 0
heroImage: "/images/anc-greece-hero.jpg"
homePreviewWidth: "clamp(9.5rem, 11.5vw, 12rem)"
homePreviewLift: "clamp(0.15rem, 1.2vw, 0.8rem)"
homePreviewTranslateX: "clamp(0.15rem, 0.35vw, 0.45rem)"
homePreviewRotate: "clamp(-1.1deg, -0.6vw, -0.35deg)"
homePreviewScale: "0.98"
gallery:
  trackGap: "clamp(3.25rem, 4.8vw, 5.75rem)"
  trackInset: "clamp(1rem, 1.35vw, 1.7rem)"
  imageWidths:
    full: "min(38vw, 44rem)"
    wide: "min(32vw, 36rem)"
    narrow: "min(20vw, 23rem)"
images:
  - src: "/images/anc-greece-hero.jpg"
    alt: "Black and white editorial layout study for Atelier Index."
    layout: "full"
    offsetY: "clamp(3rem, 8vw, 6rem)"
  - src: "/images/anc-greece-detail.jpeg"
    alt: "Constrained typographic detail grid for Atelier Index."
    layout: "wide"
  - src: "/images/anc-greece-system.jpg"
    alt: "Narrow system diagram for Atelier Index."
    layout: "narrow"
    offsetY: "clamp(9rem, 16vw, 14rem)"
---

Atelier Index treats the portfolio as an archive rather than a pitch deck. The system uses large serif headlines, compressed metadata, and image fields that carry the pace of the page.

The result is austere but not empty: every divider, margin, and typographic shift is doing a job.
